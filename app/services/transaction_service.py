...
class TransactionService:
    TYPE_LABELS = {'income': 'Доход', 'expense': 'Расход', 'opening_balance': 'Начальный баланс'}
    TYPE_EMOJI = {'income': '➕', 'expense': '➖', 'opening_balance': '💰'}

    @staticmethod
    def type_label(tx_type: str) -> str:
        return TransactionService.TYPE_LABELS.get(tx_type, tx_type)

    @staticmethod
    def type_emoji(tx_type: str) -> str:
        return TransactionService.TYPE_EMOJI.get(tx_type, '•')
    ...
    @staticmethod
    async def add_transaction(platform: str, external_user_id: str, tx_type: str, amount_raw: str, comment: str | None = None, tx_date: date | None = None) -> str:
        ...
        async with SessionFactory() as session:
            ...
            return f'{TransactionService.type_emoji(tx_type)} Операция сохранена: #{tx.id} {TransactionService.type_label(tx_type)} {sign}{amount} за {tx.transaction_date.strftime("%d.%m.%Y")}.'
    ...
    async def list_operations(platform: str, external_user_id: str, limit: int = 10) -> str:
        ...
        for tx in transactions:
            ...
            lines.append(f"- #{tx.id} {tx.transaction_date.strftime('%d.%m.%Y')} {TransactionService.type_emoji(tx.type)} {TransactionService.type_label(tx.type)} {sign}{tx.amount} {tx.comment or ''}".rstrip())
        return '\n'.join(lines)
    ...
    async def edit_transaction(platform: str, external_user_id: str, tx_id: int, amount_raw: str, comment: str | None) -> str:
        ...
        async with SessionFactory() as session:
            result = await session.execute(select(Transaction).where(Transaction.id == tx_id, Transaction.user_id == user_id))
            ...
            await session.commit()
        return f'✏️ Операция #{tx_id} обновлена.'
    ...
    async def delete_transaction(platform: str, external_user_id: str, tx_id: int) -> str:
        ...
        async with SessionFactory() as session:
            result = await session.execute(delete(Transaction).where(Transaction.id == tx_id, Transaction.user_id == user_id))
            await session.commit()
